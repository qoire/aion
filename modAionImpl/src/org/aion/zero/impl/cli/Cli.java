...
public class Cli {
    ...
    public ReturnType call(final String[] args, Cfg cfg) {
        ...
        try {
            ...
            if (options.getDumpStateTest() != null) {
                long level;
                String[] parameters = options.getDumpStateTest();
                ...
                if (level < 2) { // requires a parent and grandparent
                    ...
                    RecoveryUtils.dumpStateTestData(level, parameters);
                    return EXIT;
                }
            }
    ...
    Set<String> getSkippedTasks(Arguments options, TaskPriority breakingTaskPriority) {
        ...
        if (breakingTaskPriority.compareTo(TaskPriority.DUMP_STATE_TEST) < 0
                && options.getDumpStateTest() != null) {
            skippedTasks.add("--dump-state-test");
        }
