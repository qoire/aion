...
public class AionBlockchainImpl implements IAionBlockchain {
    ...
    private ExecutionType executionTypeForAVM = ExecutionType.ASSUME_MAINCHAIN;
    private long cachedBlockNumberForAVM = 0L;
    ...
    public Pair<ImportResult, AionBlockSummary> tryToConnectAndFetchSummary(
        ...
        if (bestBlock.isParentOf(block)) {
            ...
            if (forkLevel == NO_FORK_LEVEL) {
                executionTypeForAVM = ExecutionType.ASSUME_MAINCHAIN;
                cachedBlockNumberForAVM = bestBlock.getNumber();
            } else {
                executionTypeForAVM = ExecutionType.SWITCHING_MAINCHAIN;
            ...
            if (getBlockStore().isBlockExist(block.getParentHash())) {
                ...
                if (getBlockStore().isMainChain(block.getParentHash(), parentHeight)) {
                    ...
                    executionTypeForAVM = ExecutionType.ASSUME_SIDECHAIN;
                    cachedBlockNumberForAVM = parentHeight;
                } else {
                    // side chain parent, therefore do not know the closes main chain block
                    executionTypeForAVM = ExecutionType.ASSUME_DEEP_SIDECHAIN;
